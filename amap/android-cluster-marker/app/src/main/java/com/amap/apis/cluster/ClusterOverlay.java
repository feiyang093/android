...
public class ClusterOverlay implements AMap.OnCameraChangeListener,
    ...
    /*
      将所有的 显示数据 保存起来 ，用于 最后退出的 时候删除
      每一个 元素 是一个组   一个组里面又包含 多个 ClusterItem
    * */
     ...
     * @param context
     */
    public ClusterOverlay(AMap amap, List<RegionItem> clusterItems,
...
//默认最多会缓存80张图片作为聚合显示元素图片,根据自己显示需求和app使用内存情况,可以修改数量
    ...
    /**
     * 添加一个聚合点
     * 初始化之后，有新的聚合点加入 供外部调用
     * @param item
     */
    public void addClusterItem(RegionItem item) {
  ...
  // 每一次改变 都会调用
     ...
     * 批量添加 聚合点
     *  其实就是 每一个组划 仅仅显示一个点
     * @param clusters 组列表  Cluster 就是一个组
     */
     ...
     * 可以看成是普通点的 逻辑
     *  一个组，仅仅显示一个点
     * @param cluster 一个 组 （一个组 有几个点 组成）
     */
    private void addSingleClusterToMap(Cluster cluster) {
        LatLng latlng = cluster.getCenterLatLng();// 获取 经纬度
        // 设置 marker 点参数
        MarkerOptions markerOptions = new MarkerOptions();
        markerOptions.anchor(0.5f, 0.5f)
                .icon(getBitmapDes(cluster)) //设置图片，可以自定义图片
                .position(latlng);
        // 绑定到 高德地图
        Marker marker = mAMap.addMarker(markerOptions);
        marker.setAnimation(mADDAnimation);
        marker.setObject(cluster); //把这个数据绑定到经纬度里面去
    ...
    private void calculateClusters() {
        mIsCanceled = false;
        mClusters.clear();
        LatLngBounds visibleBounds = mAMap.getProjection().getVisibleRegion().latLngBounds;// 获取可视化区域
        for (RegionItem clusterItem : mClusterItems) {
            ...
            if (visibleBounds.contains(latlng)) {
                Cluster cluster = getCluster(latlng,mClusters);//判断 要显示的点 是否 已经包含在 当前的组列表里面，如果存在则获取符合范围的组
                if (cluster != null) {
                    cluster.addClusterItem(clusterItem);
                } else {
                    cluster = new Cluster(latlng);
                    mClusters.add(cluster); // 添加一个组
                    cluster.addClusterItem(clusterItem);
                }
    ...
    private void calculateSingleCluster(RegionItem clusterItem) {
        ...
        /**
         * 是否 有依附的点，如果有，则把依附的点，的样式更新 变成 所要展示的点的样式
         * 如果没有，则新添加 mark 点 到地图上面去
         */
     ...
     * 类似与 排重，如果 已经存在了，则不需要 加入了，直接返回最近的点
     * @param latLng
     * @return
     */
    private Cluster getCluster(LatLng latLng,List<Cluster>clusters) {
        for (Cluster cluster : clusters) {
            LatLng clusterCenterPoint = cluster.getCenterLatLng();// 获取一个组的中心点
     ...
     * 获取每个聚合点的绘制样式
     */
    private BitmapDescriptor getBitmapDes(Cluster  custer) {
//        LatLng latLng= custer.getClusterItems().get(0).getPosition();
        if (custer.getClusterItems()==null || custer.getClusterItems().size()<=0){
            return null;
        }
        RegionItem temp=  custer.getClusterItems().get(0);
        BitmapDescriptor bitmapDescriptor = mLruCache.get(temp.getUuidKey());
        if (bitmapDescriptor == null)
        {
            /**
             *  将 textVIew 转换成  图片 格式展示
             */
...
//            } else
            ...
            mLruCache.put(temp.getUuidKey(), bitmapDescriptor);// 临时保存起来，为了避免内存泄漏，最后 退出时销毁
...
//-----------------------辅助内部类用---------------------------------------------
    ...
    class MarkerHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            ...
            switch (message.what) {
                case ADD_CLUSTER_LIST: // 批量添加 market 点
                    List<Cluster> clusters = (List<Cluster>) message.obj;
                    addClusterToMap(clusters);
                    break;
                case ADD_SINGLE_CLUSTER: // 单个添加 market 点
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
                case CALCULATE_CLUSTER:
                    calculateClusters();
                    break;
                case CALCULATE_SINGLE_CLUSTER:// 添加单个 聚合点
